package migrate

import (
	"crynux_as/migrate/migrations"

	"github.com/go-gormigrate/gormigrate/v2"
	log "github.com/sirupsen/logrus"
	"gorm.io/gorm"
)

var migrationScripts []*gormigrate.Gormigrate

func Migrate() error {
	for _, migrationScript := range migrationScripts {
		if err := migrationScript.Migrate(); err != nil {
			log.Errorf("Migrate: %v", err)
			return err
		}
	}

	return nil
}

func Rollback() error {
	lastMigration := migrationScripts[len(migrationScripts)-1]

	if err := lastMigration.RollbackLast(); err != nil {
		log.Errorf("Migrate: %v", err)
		return err
	}

	return nil
}

func InitMigration(db *gorm.DB) {
	db.Set("gorm:table_options", "CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci")

	// Add new migrations here
	migrationScripts = append(migrationScripts, migrations.M20260723(db))
	migrationScripts = append(migrationScripts, migrations.M20260724(db))
	migrationScripts = append(migrationScripts, migrations.M20260725(db))
	migrationScripts = append(migrationScripts, migrations.M20260729(db))
	migrationScripts = append(migrationScripts, migrations.M20260825(db))
	migrationScripts = append(migrationScripts, migrations.M20260826(db))
	migrationScripts = append(migrationScripts, migrations.M20260907(db))
	migrationScripts = append(migrationScripts, migrations.M20260908(db))
	migrationScripts = append(migrationScripts, migrations.M20260910(db))
	migrationScripts = append(migrationScripts, migrations.M20260916(db))
	migrationScripts = append(migrationScripts, migrations.M20260917(db))
	migrationScripts = append(migrationScripts, migrations.M20260918(db))
}
