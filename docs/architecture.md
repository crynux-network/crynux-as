...
| `projects` | User projects; unique `endpoint_token` forms the private base URL; stores the project API key hash, public prefix, Cost Level `priority_gwei`, `last_request_at`, and current-Unix-day usage summary fields (`request_count_day`, `success_count_day`, `failure_count_day`, `credits_day`) |
