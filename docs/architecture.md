...
flowchart LR
  ...
  PriorityCache --> API
  ExecTimeCache --> API
  Stats[Usage Stats Workers] --> Records
  Stats --> Usage[(usage stats tables)]
```
...
`main.go` initializes components in this order: config (`config.InitConfig`), logging (`config.InitLog`), database (`config.InitDB`), database migrations (`migrate.InitMigration` + `migrate.Migrate`), blockchain clients (`blockchain.Init`), background workers (blockchain processors, stats tasks, LLM job worker, LLM job retention cleanup), the initial loaded-models and queued-priority cache refreshes and their periodic refresh tasks, and finally the HTTP server.
...
| `credit_events` | Append-only Credits ledger; every balance change is one event referencing its source record by `ref_id`; unique by type + `ref_id`; LLM charge amounts are the sole permanent store of charged Credits |
...
| `projects` | User projects; unique `endpoint_token` forms the private base URL; stores the project API key hash, public prefix, Cost Level mode (`static` or `auto`), static `priority_gwei`, auto `auto_queue_position` and `auto_max_priority_gwei`, `last_request_at`, current-Unix-day usage summary fields (`request_count_day`, `success_count_day`, `failure_count_day`, `credits_day`), and recent-failure-window counts (`recent_window_success_count`, `recent_window_failure_count`) |
| `llm_jobs` | Temporary execution rows for chat completions, completions, and responses; stores canonical task args, Bridge client task ID, execution status, raw and formatted results, billing status, execution-time coefficients, task-fee fields, and optional public response ID; retained for `llm.job_retention_days` after terminal settle |
| `llm_call_records` | Permanent finished-call rows with account snapshot `user_id`, optional unique `llm_job_id`, token usage, `token_usage_applicable`, status, `accepted_at`, `completed_at`, `duration_ms = completed_at - accepted_at`, billed effective VRAM, optional task-fee fields, and Credits recalculation snapshots; MUST NOT store request or result bodies or charged Credits amounts |
| `account_usage_hourly_stats` | Per-account Unix-hour aggregates of requests, success, failure, tokens, and charged Credits |
| `project_usage_hourly_stats` | Per-project Unix-hour aggregates with the same metrics plus `user_id` |
| `project_model_usage_10m_stats` | Per-project per-model Unix 10-minute aggregates |
| `project_duration_usage_10m_stats` | Per-project Unix 10-minute completion-duration small-bucket counts |
| `project_model_usage_snapshots` | Per-project Top-10 model snapshots for `1h`, `1d`, and `7d` |
| `project_duration_histogram_snapshots` | Per-project displayable completion-duration histogram snapshots for `1h`, `1d`, and `7d` |
| `usage_stats_progress` | Base aggregation cursor (`project_id = 0`) and per-project snapshot dirty/claim generations |
...
3. The API server resolves the request effective `priority_gwei` from the project Cost Level mode (static stored value, or auto linear position within the current queue bounds capped by `auto_max_priority_gwei`), fetches and validates LLM execution-time coefficients, computes one shared `billable_gwei` from that effective priority, rejects with HTTP 402 when Credits precheck fails, sets `task_fee_gwei = floor(billable_gwei)`, resolves a Queue Median Hint snapshot, and persists the effective `priority_gwei`, fee fields, and coefficients on the LLM job. Coefficient fetch or validation failure MUST return HTTP 500, MUST record a failed call without fee fields, and MUST NOT create an LLM job for forwarding. The live median MUST NOT enter `billable_gwei`. For static mode, live queue bounds MUST NOT enter `billable_gwei`. For auto mode, live queue bounds MAY determine the effective `priority_gwei` at job create; after that snapshot, later queue changes MUST NOT alter the job charge.
...
6. The charge is calculated from the raw result `usage` token counts, the job `priority_gwei` snapshot, the persisted execution-time coefficients, `vram_weight`, and `credits_per_gwei`. One `llm_call_records` row records the call with tokens, the `priority_gwei` used for the charge, billed effective VRAM, pre-submit task-fee fields, and the Credits recalculation snapshot. The actual Credits debit MUST be stored only on a processed LLM `credit_events` row when settle charges the account. Successful settle and failed terminal recording MUST each commit the job row lock, call record, ledger changes when charged, and job terminal update in one database transaction. The complete ownership and transaction rules are specified in [llm-job-processing.md](./llm-job-processing.md).
7. The usage stats base worker aggregates finished call records into hourly and 10-minute stats tables using processed LLM charge ledger amounts and updates `projects.last_request_at`. The snapshot worker refreshes model Top-10 and completion-duration histogram snapshots from the 10-minute tables, writes current-Unix-day summary fields onto `projects` from `project_usage_hourly_stats`, writes recent-failure-window success and failure counts onto `projects` from `project_model_usage_10m_stats`, refreshes a bounded aging batch for those recent-window counts, and clears stale day summary fields after the Unix day boundary.
