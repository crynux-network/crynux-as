...
* `GET /v1/account/charges` MUST return the charged LLM calls of the authenticated wallet, newest first, with offset/limit pagination. A charged call is a processed LLM `credit_events` row for that user. The handler MUST select the page of events first through `idx_credit_events_user_type_status_id` with `ORDER BY id DESC` and `LIMIT`/`OFFSET`, then join only that bounded event set to `llm_call_records` by `ref_id` and to `projects` by `project_id` in one SQL statement. Each item MUST include `id`, `created_at`, `project_id`, `project_name`, `model`, `prompt_tokens`, `completion_tokens`, `total_tokens`, `token_ratio`, `credits`, `billed_vram`, `duration_ms`, and `status`. The response `credits` field MUST equal the event amount. The `token_ratio` MUST be the display float of the project cost level stored on the call record at charge time. The `project_name` MUST be the current `projects.name` for the call record `project_id`, or an empty string when the project row is missing.
