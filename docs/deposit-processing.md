...
* `GET /v1/account/charges` MUST return the charged LLM calls of the authenticated wallet, newest first, with offset/limit pagination. A charged call is a processed LLM `credit_events` row for that user. The handler MUST select events first through the user/type/status/id index, then join only those event `ref_id` values to `llm_call_records`. Each item MUST include `id`, `created_at`, `project_id`, `model`, `prompt_tokens`, `completion_tokens`, `total_tokens`, `token_ratio`, `credits`, `billed_vram`, `duration_ms`, and `status`. The response `credits` field MUST equal the event amount. The `token_ratio` MUST be the display float of the project cost level stored on the call record at charge time.
